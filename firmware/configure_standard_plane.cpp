...
float ConfigureStandardPlane::Bilinear_interpolation(float x, float y, cv::Mat& mapping)
{
...
bool ConfigureStandardPlane::getStandardPlaneParam(float* ptr_point_cloud, uchar* bright,float* R,float* T)
{

    cv::Mat pointcloud_map(image_height_,image_width_,CV_32FC3,ptr_point_cloud);
    cv::Mat bright_map(image_height_,image_width_,CV_8UC1,bright);
    ...
    if (!found)
	{
		...
		bilinearInterpolationFeaturePoints(undist_circle_points, point_3d, pointcloud_map);

		 
		cv::Mat pc1(point_3d.size(), 3, CV_32F, cv::Scalar(0));
		cv::Mat pc2(point_3d.size(), 3, CV_32F, cv::Scalar(0));

		std::vector<cv::Point3f> world_points = generateAsymmetricWorldFeature(20, 10);

		for (int r = 0; r < point_3d.size(); r++)
		{
			pc2.at<float>(r, 0) = point_3d[r].x;
			pc2.at<float>(r, 1) = point_3d[r].y;
			pc2.at<float>(r, 2) = point_3d[r].z;
		}
		for (int r = 0; r < world_points.size(); r++)
		{
			pc1.at<float>(r, 0) = world_points[r].x;
			pc1.at<float>(r, 1) = world_points[r].y;
			pc1.at<float>(r, 2) = world_points[r].z;
		}
...
// arr1: Mat (1, N) CV_32F
// arr2: Mat (1, M) CV_32F
// return: Mat (N, M) CV_32F
cv::Mat ConfigureStandardPlane::kronProductArr(const cv::Mat& arr1, const cv::Mat& arr2)
{
	int N = arr1.cols;
	int M = arr2.cols;
	cv::Mat arr1Expand = repeat(arr1.t(), 1, M);
	cv::Mat arr2Expand = repeat(arr2, N, 1);
	cv::Mat kroProduct = arr1Expand.mul(arr2Expand);
	return kroProduct;
}
...
// r: Mat (3, 3) CV_32F
// t: Mat (3, 1) CV_32F
void ConfigureStandardPlane::svdIcp(const cv::Mat& pc1, const cv::Mat& pc2, cv::Mat& r, cv::Mat& t)
{
	int pointNum = pc1.rows;
	cv::Mat pc1Mean = pcMean(pc1);
	cv::Mat pc2Mean = pcMean(pc2);
	cv::Mat pc1Centered = pc1 - repeat(pc1Mean, pointNum, 1);
	cv::Mat pc2Centered = pc2 - repeat(pc2Mean, pointNum, 1);
	cv::Mat w = cv::Mat::zeros(3, 3, CV_32F);
	for (int i = 0; i < pointNum; i++) {
		cv::Mat q1 = pc1Centered(sliceMask(i, -1, 0, 3));
		cv::Mat q2 = pc2Centered(sliceMask(i, -1, 0, 3));
		w += kronProductArr(q2, q1);
	}
	cv::Mat u, sigma, vt;
	cv::SVD::compute(w, sigma, u, vt);
	r = vt.t() * u.t();
	float det = determinant(r);
	if (det < 0) {
		vt.at<float>(2, 0) = -vt.at<float>(2, 0);
		vt.at<float>(2, 1) = -vt.at<float>(2, 1);
		vt.at<float>(2, 2) = -vt.at<float>(2, 2);
		r = vt.t() * u.t();
	}
...
cv::Rect ConfigureStandardPlane::sliceMask(int rowStart, int rowEnd, int colStart, int colEnd)
{
...
// pc: Mat (N, 3) CV_32F
// return: pcMean (1, 3) CV_32F
cv::Mat ConfigureStandardPlane::pcMean(const cv::Mat& pc)
{
	cv::Mat xList = pc(sliceMask(0, pc.rows, 0, -1));
	cv::Mat yList = pc(sliceMask(0, pc.rows, 1, -1));
	cv::Mat zList = pc(sliceMask(0, pc.rows, 2, -1));
	cv::Mat meanXYZ =
		(cv::Mat_<float>(1, 3) << mean(xList)[0],
			mean(yList)[0],
